...
const App = () => {
	...
	const [ basketStatus, setBasketStatus ] = useState(JSON.parse((localStorage.getItem('basketStatus') || 'null')) || {});
			...
			<Switch>
				...
				<Route path="/basket/:areaId/:itemId" exact>
					<Basket
						...
						setupBasketStatus={({itemId, ...data}) => {
							const nextStatus = {...basketStatus}

							nextStatus[itemId] = data

							setBasketStatus(nextStatus)
							localStorage.setItem('basketStatus', JSON.stringify(nextStatus));
						}}
					/>
				</Route>
				<Route
				...
				>
					<Orders 
						...
						setFinishedOrder={({ itemId }) => {
							...
							nextStatuses[itemId] = 'CANCELED'; // fixed #4
